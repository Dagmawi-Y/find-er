<script>
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      if (first_name_error) toast.error("First name is required");
      if (last_name_error) toast.error("Last name is required");
      if (email_error) toast.error("Please enter a valid email address");
      if (password_error) toast.error("Password is required");
      if (confirm_password_error) toast.error("Passwords don't match");
      return;
    }
      ...
      <form class="space-y-4 md:space-y-6" on:submit={handleSubmit}>
        ...
        >
          {#if form_submitted}
            ...
            <span class="loading loading-spinner text-primary-light"></span>
          {:else}
            Create an account
          {/if}
        </button>
